---
title: Introducing Metharme-2
seo_title: Metharme-2
summary: 
slug: introducing_metharme2
author: TearGosling
...
It's been months upon months since a major announcement like this, but we've finally done it: new model releases. Introducing our new models: Metharme-2 in **7B**, **13B** and **33B** sizes.
...
With that out of the way, let's get straight into the details of how we've trained our newest models, [MODEL NAMES HERE]. We'll go over both the dataset used to train our models and the process of training the models themselves.
...
Let's take a more detailed look at our dataset, comprised of a staggering 1.4 billion tokens in total:
<image>
We will divide and go over the sources of our datasets in detail from two sides: the ***creative*** side and the ***instructional*** side.

### Creativity
- **Roleplay forums** *(24.8%)*
...
- [**PIPPA**](https://huggingface.co/datasets/PygmalionAI/PIPPA) *(6.6%)*
...
- [**Club Floyd**](https://wandb.ai/ve-forbryderne/skein/runs/files/files/datasets/floyd) *(1.7%)*
...
- [**text_adventures.txt**](https://github.com/Latitude-Archives/AIDungeon/blob/ca098ca7dab480d24e47954c8873b03ba1091ffc/data/text_adventures.txt) *(0.8%)*
...
### Instructional
- [**OpenOrca**](https://huggingface.co/datasets/Open-Orca/OpenOrca) *(31.4%)*
...
- [**Multiround Claude Instructions**](https://huggingface.co/datasets/Norquinal/claude_multiround_chat_30k) *(8.2%)*
...
- [**Airoboros**](https://huggingface.co/datasets/jondurbin/airoboros-gpt4-1.4.1) *(2.4%)*
...
We have released fine-tunes of the **Llama-2** base model in two sizes: 7B and 13B. Our 7B model was trained on 8x A40s graciously provided by [Arc Compute](https://www.arccompute.io/) on our full 1.4B token dataset, while our 13B model was trained on 8x H100s loaned to us from [Google Cloud](https://cloud.google.com). We extend our heartfelt thanks to Dr. Yinhe Zheng for being able to provide these GPUs to train the 13B with.

Though we initially used our own repo for training the models, we later switched to the [Axolotl](https://github.com/OpenAccess-AI-Collective/axolotl) repo for easier and more efficient training.

Our 7B, 13B and 33B models have been trained with the following hyperparameters:
- Micro batch size of 8 on each GPU, resulting in a global batch size of 64.
- Learning rate of 1.7e-5, with a cosine learning rate scheduler and 32 warmup steps (applied quadratically)
- AdamW optimizer with betas of (0.9, 0.95) and a weight decay of 0.1
- Trained in bfloat16 precision.

<insert 7b image here>

*Eval loss chart of our final 7B model run.*

<insert 13b image here>

*Eval loss chart of our final 13B model run.*

<insert 33b image here>

*Eval loss chart of our 33B model run.*